...
colHeaders_types = {
    ...
    "Year One Bill_OPTIMIZED [$]": float,
    "Year One Bill Savings [$]": float,

    "Analysis Period Emissions_BAU [tonnes CO2]": float,
    "Analysis Period Emissions_OPTIMIZED [tonnes CO2]": float,
    ...
    "Annual Emissions_OPTIMIZED [tonnes CO2]": float,
    "Annual Emissions Savings [tonnes CO2]": float,
    "Avoided Health Damage Costs [$]": float
...
def SaveOutputs(res, name, colHeaders):
    building = name.split("building", 1)[1]
    ...
    res = np.append(res, tariff["year_one_bill_before_tax_bau"])   # "Year One Bill_BAU [$]",
    res = np.append(res, tariff["year_one_bill_before_tax"])    # "Year One Bill_OPTIMIZED [$]",
    ...
    res = np.append(res, site["lifecycle_emissions_tonnes_CO2"])    # "Analysis Period Emissions_OPTIMIZED [tonnes CO2]",
    ...
    res = np.append(res, site["annual_emissions_tonnes_CO2"])    # "Year One Emissions_OPTIMIZED [tonnes CO2]",
...
list_of_columns = ["Building Name", "NPV [$]", "LCC BAU [$]", "LCC [$]", "NPV [$]", "lifecycle capital costs",
                   "Avoided Health Damage Costs [$]", "lifecycle capital costs", "Annual Emissions Savings [tonnes CO2]"]

excel_filename = "GHP_main_results.xlsx"
